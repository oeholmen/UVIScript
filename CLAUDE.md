...
The `compile.sh` script inlines all `includes/` dependencies into standalone files and writes them to the `compiled/` folder. It also copies resources and program patches.
...
The script will:
1. Copy the `resources/` folder into `compiled/resources/`.
2. Copy the Tweak Synth `.uvip` program files into `compiled/synths/programs/`.
3. For each listed Lua script, inline the required includes and write a `*Compiled.lua` file under `compiled/`.
4. Generate additional variants for some scripts (e.g. mono/multipart versions of sequencers, 8- and 16-part MIDI control scripts).
